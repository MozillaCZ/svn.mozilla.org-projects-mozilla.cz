<?php
	...
	$page->setTitle('Mozilla Suite &ndash; Poznámky k vydání verze 1.3a');
	$page->setHeadline('Poznámky k vydání','Popis změn a novinek ve verzi 1.3a');
	$page->setKeywords ('mozilla, suite, poznámky k vydání, verze, gecko, prohlížeč, poštovní klient, editor, chatzilla');
	$page->setDescription ('Poznámky k vydání historické verze 1.3a');
	$page->setMenu('produkty');
	$page->includeTemplate('header');
?>

<p>Tyto <a href="/produkty/suite/poznamky-k-vydani/">poznámky k verzi</a> popisují systémové požadavky, pokyny k instalaci a známé problémy týkající se Mozilly verze 1.3a.</p>
...
<h2 id="new">Co je nového v Mozille 1.3a</h2>
<ul>
	<li>Mozilla 1.3a obsahuje mnoho nových vlastnostní a oprav v Poště a diskusních skupinách.
	<ul>
		<li>Mozilla Mail nyní obsahuje filtr pro třídění nevyžádané pošty. Znamená to, že můžete naučit poštovního
		klienta rozlišovat mezi klasickou a nevyžádanou poštou [spam, junk-mail, atd.]. V této verzi
		filtr pro třídění nevyžádané pošty označí automaticky nevyžádanou poštu jako junk, ale její automatické smazání není povoleno. Více o této nové vlastnosti se můžete dočíst na stránce <a href="http://www.mozilla.org/mailnews/spam.html">Spamové filtry Mozilly</a> [anglicky].</li>
		<li>Tato verze rovněž obsahuje nové nástroje pro filtrování. Konkrétně se jedná o hromadné akce pro filtry, takže můžete provádět činnosti jako je přesouvání a označování najednou. Rovněž jsou implementovány filtry pro diskusní skupiny.</li>
		<li><span class="HIDETODO"><a href="http://www.mozilla.org/mailnews/specs/views/">Zobrazení zpráv</a></span> je nová vlastnost Mozilly, která usnadňuje uživateli hledání, organizování a třídění svých e-mailových zpráv. <b>Zobrazení</b> je použito k filtrování a zobrazení zpráv splňujících jistá kritéria. Mozilla je standardně vybavena sadou předdefinovaných <b>zobrazení</b>. Uživatelé si mohou také vytvořit svá vlastní.</li>
		<li>Byla vylepšena schopnost spolupracovat s některými typy imap serverů (zvláště pak serveru iMail) a byly vyřešeny problémy s ACL na serverech Cyrus.</li>
		<li>Tým vytvářející komponentu Pošta vyřešil problém s kontrolou příchozí pošty na účtu, do kterého jste se nezalogovali a se zamrzáním programu při hledání v lokálně uložených zprávách.</li>
	</ul></li>
	<li>Záložky nyní obsahují pohodlné a rychlé vyhledávání podobné tomu v poště a diskusních skupinách (a v prohlížeči Phoenix). Jedná se o možnost rychle filtrovat záložky zadáním několika znaků do hledacího pole. Hledací pole se nachází v horní části okna se záložkami. Zadaná klíčová slova se porovnávají s názvy jednotlivých záložek.</li>
	<li>Do verze pro operační systém Windows byla zahrnuta podpora Intellimouse Explorer pro tlačítka Zpět a Vpřed.</li>
	<li>Uživatelé komponenty DOM Inspektor mohou nyní vypnout zobrazování prázdných uzlů [nodes] typu #text</li>
	<li>Změnili jsme stav verzí pro Mac Classis (OS 8.x a 9.x) v dokumentu <a href="http://www.mozilla.org/ports/">Mozilla ports status</a>. Znamená to, že počínaje verzí 1.3alfa, mozilla.org nebude vytvářet klasické oficální verze. Uživatelé OS 9 s carbonlib mohou u verze 1.3alfa (ale pravděpodobně již ne následujících verzí) použít OS X (CFM) verzi. Pro použití CFM OSX verzí na OS 9 postupujte takto: Stáhněte a rozbalte binárku. Otevřete disk image. Přetáhněte Mozilla adresář na váš desktop. Otevřete adresář a dvojklikněte na soubor Mozilla.app.</li>
</ul>
...
<ul>
	<li>Výkon zasílání vložených (inline) poštovních zpráv <span class="HIDETODO"> se zhoršil</span>. To jest již opraveno ve verzi 1.3beta, proto prosím neohlašujte tuto chybu. (chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=184550">184550</a>)</li>
	<li>Na Mac OS X se uživatel může dostat do stavu, kdy je okno prohlížeče vyplněno šedou barvou. Obvykle k tomu dochází, když předešlým vybraným prvkem byl <span class="HIDETODO">obrysový grafický prvek</span><!-- outliner widget, jak to prelozit lepe? --> (např. okno historie nebo okno vláken v poště a diskusních skupinách). (chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=181293">181293</a>)</li>
	<li>Za některých okolností nepůjde Mozilla spustit s některými uživatelskými profily.
	<ul>
		<li>Pokud jste používali téma vzhledu jiné nežli "Classic" nebo "Modern", tak to může být způsobeno špatným záznamem v souboru <tt>chrome/chrome.rdf</tt> v uživatelském profilu. Zkuste nalézt v souboru <tt>&lt;ProfileDirectory&gt;/chrome/chrome.rdf</tt> řádky podobné těmto:<br />
<tt>
...
<br />Pokud '<tt>selectedSkin</tt>' obsahuje něco jiného než '<tt>...skin:modern/1.0:...</tt>' a Mozilla nelze spustit, upravte tento soubor, pokud Mozilla právě neběží a změňte tuto položku tak, aby obsahovala '<tt>modern/1.0</tt>'.</li>
	...
	<li>Vyhledávání Ebay může způsobit pád aplikace nebo její zamrznutí pod OS/2 (a někdy i MS Windows?). (chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=184931">184931</a>)</li>
</ul>

<h2 id="require">Požadavky na systém</h2>
<h3>Macintosh</h3>
<ul>
	<li>Mac OS 9.1 nebo novější</li>
	<li>Procesor PowerPC (doporučujeme 266 MHz nebo rychlejší)</li>
	<li>64 MB RAM</li>
	<li>36 MB volného místa na disku</li>
	<li>QuickTime (chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=59059" title="Bug 59059 - With Quicktime extension disabled, Mac N6 will not launch">59059</a>)</li>
	<li>ContextualMenu knihovna (chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=150018" title="Bug 150018 - Mozilla/Installer requirement not listed: ContextualMenu Library">150018</a>)</li>
</ul>

<h3>Windows</h3>
<ul>
	<li>Windows 95, 98 nebo 98SE, Windows ME, Windows NT 4.0, Windows 2000 nebo Windows XP</li>
	<li>Procesor třídy Intel pentium 233 MHz (nebo rychlejší)</li>
	<li>64 MB RAM</li>
...
<h3>Mac OS</h3>
<p>Pokud používáte StuffIt Expander verze 4.5 (nebo starší), můžete pozorovat chybovou zprávu říkající, že archiv jest poškozen. Tuto zprávu můžete ignorovat a pokračovat v instalaci. Novou verzi StuffIt Expanderu získáte z <a href="http://www.aladdinsys.com/expander/expander_mac_login.html">aladdinsys.com</a>.</p>
...
<p>Pokud používáte SmoothType kontrolní panel místo Applovské volby "<acronym title="Smooth all fonts on screen">Vyhladit všechny fonty na obrazovce</acronym>" v kontrolním panelu <acronym title="Appearance">Vzhled</acronym>, musíte mít SmoothType verze 2.1.1 pro Mozillu. Mozilla není kompatibilní v dřívějšími verzemi SmoothType a některé stránky mohou být zobrazeny nesprávně. Update získáne z <a href="http://www.kaleidoscope.net/greg/smoothtype.html">kaleidoscope.net</a>.</p>
...
<p>Uživatelé Red Hatu 7, kteří mají problémy se zamrzáním Javy, mohou zkusit nastavit proměnnou prostředí LD_ASSUME_KERNEL na hodnotu "2.2.5". (chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=84093" title="Bug 84093 - hangs on java, RedHat 7.1">84093</a>)</p>
...
<p><strong>Mac OS:</strong> Instalace Mozilly požaduje některá systémová rozšíření zahrnující QuickTime, proto byste neměli spuštět Mozillu, pokud spouštíte váš sytém bez všech rozšíření.</p>
...
<p><strong>Windows:</strong> Pokud používáte verze Mozilly s instalátorem a již máte JRE 1.3.0_01 na vašem systému, Mozilla by to měla automaticky poznat. Pokud jste ručně nastavili zákaz automatického hledání pluginů, tak pro zprovoznění Javy učiňte následující krok. Po instalaci Javy na váš počítač zkopírujte soubor <tt>NPOJI610.dll</tt> z instalačního adresáře (obvykle něco jako <tt>C:\Program Files\JavaSoft\JRE\1.3.0_01\bin</tt>) do adresáře pluginů Mozilly (obvykle <tt>C:\Program Files\Mozilla 1.3a\plugins</tt>).</p>
...
<p><strong>Mac OS:</strong> Musíte mít Mac OS Runtime for Java (MRJ) verze 2.2 pro spouštění appletů v Mozille. Pokud máte verzi Mac OS 8.5 až 8.6, můžete potřebovat upgrade vaší verze MRJ. Upgrade můžete získat z <a href="http://www.versiontracker.com/moreinfo.fcgi?id=377&amp;db=mac">MRJ stránky</a> serveru <a href="http://www.versiontracker.com/">versiontracker</a>.</p>
...
<ol>
	...
	<li>Extrahujte soubory z archivu do adresáře např. <tt>C:\Program Files\Mozilla 1.3a</tt> .</li>
	<li>Mozillu spustíte dvojklikem na soubor Mozilla.exe v aresáři, do kterého jste extrahovali soubory z archivu.</li>
</ol>

<h3 id="macinstall">Instalace na Mac OS</h3>
<p><strong>Note:</strong> Pokud se stažený soubor nerozbalí automaticky, musíte jej rozbalit pomocí programu StuffIt Expander. Expander je dodáván s každým novým systémem Mac a nachází se v standardní instalaci systému. Hledáním "StuffIt" pomocí služby Sherlock se můžete přesvědčit, zda jej máte instalován.</p>
<p><a href="http://www.aladdinsys.com/expander/expander_mac_login.html">StuffIt Expander</a> můžete získat od <a href="http://www.aladdinsys.com/">Aladdin Systems</a>.</p>
<p>Pokud již Expander nemáte, budete jej muset nastavit, aby se spouštěl standardně. Na předchozí URL naleznete infomace o používání programu Expander.</p>
<p>Při instalování Mozilly prostřednictvím staženého installátoru doporučujeme postupovat podle následujících kroků:</p>
<ol>
	<li>Klikněte na odkaz na soubor <tt>MacMozillaInstaller.sea.bin</tt>, čímž stáhnete tento soubor na váš počítač. Standardně se soubor nahraje na váš desktop v komprimovaném formátu .bin. Je vytvořen adresář pro instalátor mozilly.</li>
	<li>Dvojklikněte na adresář s instalátorem Mozilly ve Finderu.</li>
	<li>Dvojklikněte na ikonu instalátoru Mozilly.</li>
	<li>Řiďte se instrukcemi na obrazovce během celé instalace.</li>
	<li>Mozillu spustíte dvojklikem na ikonu Mozilly v adresáři, do kterého jste Mozillu nainstalovali.</li>
</ol>
<p>Pokud instalujete Mozillu ze staženého archivu (ruční instalace) postupujte dle následujících kroků:</p>
<ol>
	<li>Klikněte na odkaz na soubor mozilla-mac-*.sea.bin, čímž nahrajete soubor na váš počítač. Standardně se soubor nahraje na váš desktop v komprimovaném formátu .sit.bin.</li>
	<li>Dvojklikněte na ikonu mozilla-mac-*.sea.sit.bin na vašem desktopu.</li>
	<li>Vyberte adresář, do kterého si přejete instalovat Mozillu.</li>
	<li>Mozillu spustíte otevřením adresáře, do kterého jste ji nainstalovali, a následným dvojkliknutím na ikonu Mozilly.</li>
</ol>
...
<ol>
	<li>Vytvořte adresář mozilla1.3a (<code>mkdir mozilla1.3a</code>) a přemístěte se do tohoto adresáře (<code>cd mozilla1.3a</code>).</li>
	<li>Klikněte na odkaz na soubor s instalátorem (jmenuje se <tt>Mozilla-i686-pc-linux-gnu-installer.tar.gz</tt>) a stáhněte jej do vytvořeného adresáře mozilla1.3a.</li>
	<li>V adresáři mozilla1.3a rozbalte archiv následujícím příkazem: <code> gunzip -dc moz*.tar.gz | tar -xvf -<br /></code> (Ten rozbalí instalátor do podadresáře <tt>mozilla-installer</tt>.)</li>
...
<ol>
	...
	<li>V Mozilla adresáři (<code>mozilla1.3a</code>) rozbalte soubor následujícím příkazem: <code> gunzip -dc moz*.tar.gz | tar -xvf -</code> (Vytvoříte tím podadresář "mozilla" v adresáři mozilla1.3a.)</li>
...
<h3>Linux</h3>
<ul>
	<li>Smažte adresář <tt>mozilla1.3a</tt>, do kterého jste nainstalovali Mozillu</li>
	<li>Smažte adresář <tt>~/.mozilla</tt>, který obsahuje infomace vašeho profilu</li>
</ul>

<h3>Windows</h3>
<ul>
	<li>V "Nastavení/Ovládací panely/Přidat nebo odebrat programy" zvolte "Mozilla 1.3a"</li>
</ul>

<h3>Mac OS 9</h3>
<ul>
	<li>Smažte adresář mozilla z adresáře Documents</li>
	<li>Smažte soubor "Mozilla Registry" z <tt>System Folder|Preferences folder</tt></li>
...
<tr><td class="deftable-item">Mac OS 9</td><td class="deftable-def">Documents:Mozilla:[Uživatelský profil]:[náhodný řetězec].slt:</td></tr>
...
<p><strong>Mac OS:</strong> Je známa necompatibilita mezi Mozillou a WebFree, kontrolním panelem používaným k blokování na HTML založených inzerátech. Pokud chcete používat Mozillu, vypněte WebFree. (chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=13982" title="Bug 13982 - WebFree (html filter)'s block blinking text causes layout to">13982</a>)</p>
...
<p><strong>(Mac OS 9.x)</strong> Správná funkce kolečka myši na zařízeních Kensington vyžaduje Kensington Mouse Works 5.7 nebo vyšší. Verze 5.7 je nyní dostupná na stránkách <a href="http://kensington.custhelp.com/cgi-bin/kensington.cfg/php/enduser/std_adp.php?p_faqid=330">Kensington</a>.</p>
...
<p>Volby u pomocných aplikací, které nejsou definovány operačním systémem si Mozilla nemusí pamatovat. Pro řešení zkuste tento postup:</p>
<ol>
	<li>V dialogovém okně Předvolby vyberte Prohlížeč | Pomocné aplikace.</li>
	<li>klikněte na tlačítko Obnovit.</li>
	<li>Uzavřete dialogové okno Předvolby kliknutím na OK.</li>
</ol>
(chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=93173" title="Bug 93173 - Helper app decision  isn't remembered [using Open for non-OS">93173</a>)
...
<h4>Pošta offline</h4>
<p>Některé položky v komponentě 'Pošta' se nezobrazují správně (předvolby Režim offline &amp; a prostor na disku, výběr voleb offline, indikátor online/offline v Hledání/Historie/Přihlásit k odebírání). (chyba <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=80970" title="Bug 80970 - offline indicators for search, history, subscribe not workin">80970</a>)</p>
...
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=177258" title="bug 177258 - [ps] Memory leak of nsFontMetricsPS and nsFontPS object i...">177258</a>,
...
<p><strong>Linux:</strong> LiveConnet nyní funguje, pokud používáte Mozillu 1.3a s JRE 1.3.1. U verze JDK 1.4 se vyskytly problémy zmiňované výše.</p>
...
<ul>
	...
	<li>Na&nbsp;Unixu: je vytvořen symbolický odkaz na <tt>/usr/java/j*/jre/plugins/i386/ns610/libjavaplugin_oji.so</tt> z podadresáře <tt>plugins/</tt> v adresáři s nainstalovanou mozillou&nbsp;1.3a. Nekopírujte soubor, pouze vytvořte symbolický odkaz.</li>
</ul>
...
<p><small>Tento dokument je překladem <a href="http://www.mozilla.org/releases/mozilla1.3a/">dokumentu z mozilla.org</a>. Copyright &copy; 1998-2002 The Mozilla Organization</small></p>
