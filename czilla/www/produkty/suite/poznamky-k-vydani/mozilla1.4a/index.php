<?php
	...
	$page->setTitle('Mozilla Suite &ndash; Poznámky k vydání verze 1.4a');
	$page->setHeadline('Poznámky k vydání','Popis změn a novinek ve verzi 1.4a');
	$page->setKeywords ('mozilla, suite, poznámky k vydání, verze, gecko, prohlížeč, poštovní klient, editor, chatzilla');
	$page->setDescription ('Poznámky k vydání historické verze 1.4a');
	$page->setMenu('produkty');
	$page->includeTemplate('header');
?>

<p>Tyto <a href="/produkty/suite/poznamky-k-vydani/">poznámky k verzi</a> popisují systémové požadavky, pokyny k instalaci a známé problémy týkající se Mozilly verze 1.4a.</p>
...
<h2 id="new">Co je nového v Mozille 1.4a</h2>
<ul>
	<li>V Mozille 1.4a bylo opraveno cca 1000 chyb, což zahrnuje mimo jiné zvýšení výkonu, stability a větší podporu standardů.</li>
	<li>Byly provedeny změny v záložkách. Záložky nyní obsahují kořenovou složku. Uživatel nyní již může mít dvě odlišně se jmenující záložky ukazující na stejnou stránku. Záložky obsahují ikony webových serverů ve správci záložek i postranní liště. Oddělovače nyní mohou obsahovat popisek.</li>
	<li>Editor nyní umožňuje dynamicky měnit velikost obrázků a tabulek pomocí myši. Pokud je vybrán obrázek nebo se textový kurzor nachází v tabulce, zobrazí se osm aktivních bodů, které umožňují upravit rozměry obrázku či tabulky tažením myší. V případě obrázku se změna velikosti zobrazuje souběžně s pohybem myši a výsledné rozměry jsou tak porozovatelné přímo jako průhledný rámeček. Zároveň je zobrazována aktuální velikost v jednotkách pixelů a relativní změna v pixelech jako <acronym title="tooltip">plovoucí tip</acronym>.</li>
	<li>Mozilla nyní umožňuje <acronym title="smooth scrolling">plynulé posouvání</acronym>. Po instalaci je tato funkce vypnuta. Pokud ji chcete zapnout, tak pomocí <code>about:config</code> přidejte do předvoleb novou "boolean" hodnotu <code>general.smoothScroll</code> a nastavte ji na "<code>true</code>". Nastavením této hodnoty na "<code>false</code>" tuto funkci opět vypnete.</li>
	<li>Poštovní klient nyní obsahuje v kontextové nabídce položky <acronym title="junk-mail">nevyžádaná pošta</acronym> a odstranit nevyžádaný mail a další vylepšení týkající se práce s nevyžádanou poštou.</li>
	<li>Blokování vyskakovacích (popup) oken bylo nyní upraveno s cílem zlepšit jeho použitelnost.</li>
	<li>XPInstall a instalace témat vzhledu byly na Mac OS X opět obnoveny.</li>
</ul>
...
<ul>
	<li>MRJ Java plugin pro tuto verzi není dostupný, proto LiveConnect nefunguje. Mozilla pro zobrazení appletů použije Applovský Java applet zásuvný modul (plugin), je-li dostupný. (<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=97613">97613</a>)</li>
</ul>
...
<ul>
	...
	<li>80 MB volného místa na disku</li>
</ul>
...
<p><strong>Windows:</strong> Pokud používáte verze Mozilly s instalátorem a již máte JRE 1.3.0_01 na vašem systému, Mozilla by to měla automaticky poznat. Pokud jste ručně nastavili zákaz automatického hledání pluginů, tak pro zprovoznění Javy učiňte následující krok. Po instalaci Javy na váš počítač zkopírujte soubor <tt>NPOJI610.dll</tt> z instalačního adresáře (obvykle něco jako <tt>C:\Program Files\JavaSoft\JRE\1.3.0_01\bin</tt>) do adresáře pluginů Mozilly (obvykle <tt>C:\Program Files\Mozilla 1.4a\plugins</tt>).</p>
...
<ol>
	...
	<li>Extrahujte soubory z archivu do adresáře např. <tt>C:\Program Files\Mozilla 1.4a</tt> .</li>
...
<ol>
	<li>Vytvořte adresář mozilla1.4a (<code>mkdir mozilla1.4a</code>) a přemístěte se do tohoto adresáře (<code>cd mozilla1.4a</code>).</li>
	<li>Klikněte na odkaz na soubor s instalátorem (jmenuje se <tt>Mozilla-i686-pc-linux-gnu-installer.tar.gz</tt>) a stáhněte jej do vytvořeného adresáře mozilla1.4a.</li>
	<li>V adresáři mozilla1.4a rozbalte archiv následujícím příkazem: <code> gunzip -dc moz*.tar.gz | tar -xvf -<br /></code> (Ten rozbalí instalátor do podadresáře <tt>mozilla-installer</tt>.)</li>
...
<ol>
	...
	<li>V Mozilla adresáři (<code>mozilla1.4a</code>) rozbalte soubor následujícím příkazem: <code> gunzip -dc moz*.tar.gz | tar -xvf -</code> (Vytvoříte tím podadresář "mozilla" v adresáři mozilla1.4a.)</li>
...
<h3>Linux</h3>
<ul>
	<li>Smažte adresář <tt>mozilla1.4a</tt>, do kterého jste nainstalovali Mozillu</li>
	<li>Smažte adresář <tt>~/.mozilla</tt>, který obsahuje infomace vašeho profilu</li>
</ul>

<h3>Windows</h3>
<ul>
	<li>V "Nastavení/Ovládací panely/Přidat nebo odebrat programy" zvolte "Mozilla 1.4a"</li>
</ul>
...
<p><strong>Linux:</strong> Funkce náhled tisku (lišty, tisk z náhledu tisku, vzhled stránky atd.) je plně funkční pouze s modulem <a href="#xprint">Xprint</a>. (chyby <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=119491" title="bug 119491 - [ps] Cleanup global vars in PostScript module (gfx/src/ps/)">119491</a>, <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=126802" title="bug 126802 - Printing while in print preview disabled on Unix">126802</a>, <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=127810" title="bug 127810 - Enable UI for zoom, print, and landscape/postscript on Unix once we can">127810</a>, <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=129507" title="bug 129507 - Page Setup from Print Preview does nothing?">129507</a>)</p>
...
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=185895" title="bug 185895 - [ps] ceiling and floor print incorrectly (rendered correctly)">185895</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=187184" title="bug 187184 - border-top under linux prints all four borders under linux but displays correctly to screen. In win2k, screen and print work correctly.">187184</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=189884" title="bug 189884 - postscript output only print an error message, not page  ">189884</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=190316" title="bug 190316 - Fine line prints around first page">190316</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=194761" title="bug 194761 - Huge static buffer in gfx/src/ps/nsPostScriptObj.cpp ">194761</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=194830" title="bug 194830 - border on last page when printing A4 postscript to a file">194830</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=195043" title="bug 195043 - cannot print 2-byte character (i.e. big-5 or gb2312) correctly.">195043</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=195056" title="bug 195056 - Mozilla freezes, when lpr quits with a warning 'jobs queued, but cannot start daemon.'">195043</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=195206" title="bug 195206 - Mozilla's PS code does poor rendering because of coordinate rounding (e.g. letter spacing)">195206</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=196306" title="bug 196306 - Mozilla goes into a loop when trying to print (even to a file)">196306</a>,
<a href="https://bugzilla.mozilla.org/show_bug.cgi?id=198165" title="bug 198165 - Print to File Using Postscript/Default Creates Unviewable PostScript Files">198165</a>)
</p>
...
<p><strong>Linux:</strong> LiveConnet nyní funguje, pokud používáte Mozillu 1.4a s JRE 1.3.1. U verze JDK 1.4 se vyskytly problémy zmiňované výše.</p>
...
<ul>
	...
	<li>Na&nbsp;Unixu: je vytvořen symbolický odkaz na <tt>/usr/java/j*/jre/plugins/i386/ns610/libjavaplugin_oji.so</tt> z podadresáře <tt>plugins/</tt> v adresáři s nainstalovanou Mozillou&nbsp;1.4a. Nekopírujte soubor, pouze vytvořte symbolický odkaz.</li>
</ul>
...
<p><small>Tento dokument je překladem <a href="http://www.mozilla.org/releases/mozilla1.4a/">dokumentu z mozilla.org</a>. Copyright &copy; 1998-2003 The Mozilla Organization</small></p>
